...
{
  EEPORM_UNIO_PORT->DDR |= EEPROM_UNIO_PIN;   /* Out Hight */
  EEPORM_UNIO_PORT->CR1 &= ~EEPROM_UNIO_PIN;  /* Open Drain */
  EEPORM_UNIO_PORT->CR2 &= ~EEPROM_UNIO_PIN;  /* Output Speed 2MHz */ 
  EEPORM_UNIO_PORT->ODR |= EEPROM_UNIO_PIN;  /* Output model */
}
...
{ 
  SET_IO_LOW(EEPORM_UNIO_PORT,EEPROM_UNIO_PIN);        /* SCIO Output 0*/
  Delay_us(30);
  SET_IO_HIGH(EEPORM_UNIO_PORT,EEPROM_UNIO_PIN);        /* SCIO Output 1*/
  Delay_us(30);
}
...
{ 
  ...
  while(EEPORM_UNIO_PORT->IDR & EEPROM_UNIO_PIN)
  {
    timeout--;
    if(timeout == 0)
    {
  ...
  Delay_us(60);
  if(!(EEPORM_UNIO_PORT->IDR & EEPROM_UNIO_PIN))
  {
    EEPROM_ERROR_Flag = 1;
...
{ 
  ...
  SET_IO_HIGH(EEPORM_UNIO_PORT,EEPROM_UNIO_PIN);
  while(!(EEPORM_UNIO_PORT->IDR & EEPROM_UNIO_PIN))        /* if SCIO is 0 */
  {
    timeout--;
    if(timeout == 0)
    {
...
void EEPROM_Write_Byte(uint8_t write_data)
{
  ...
  while(t)
  {
    if(write_data & t)
    {
      SET_IO_LOW(EEPORM_UNIO_PORT,EEPROM_UNIO_PIN);
      Delay_us(30);
      SET_IO_HIGH(EEPORM_UNIO_PORT,EEPROM_UNIO_PIN);
      Delay_us(30);
    }else{
...
* times to EEPORM.
*
* Arguments  : The 16bit number data.
*
* Return : none.
*
* Note(s)    : If timeout, Than set EEPROM ERROR Flag.  
...
uint32_t Read_Num_TO_EEPROM(void)
{
  ...
  //read_data = read_arry[3];  /* Two 8 bit data merge one 16 bit data */
